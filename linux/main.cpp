class AirPodsTrayApp : public QObject {
 ...
 public:
  AirPodsTrayApp(bool debugMode) : debugMode(debugMode) {
    ...
    QAction *caToggleAction =
        new QAction("Toggle Conversational Awareness", trayMenu);
    ...
    QAction *noiseCancellationAction =
        new QAction("Noise Cancellation", trayMenu);
    ...
    connect(offAction, &QAction::triggered, this,
            [this]() { setNoiseControlMode(0); });
    connect(transparencyAction, &QAction::triggered, this,
            [this]() { setNoiseControlMode(2); });
    connect(adaptiveAction, &QAction::triggered, this,
            [this]() { setNoiseControlMode(3); });
    connect(noiseCancellationAction, &QAction::triggered, this,
            [this]() { setNoiseControlMode(1); });

    connect(this, &AirPodsTrayApp::noiseControlModeChanged, this,
            &AirPodsTrayApp::updateNoiseControlMenu);
    connect(this, &AirPodsTrayApp::batteryStatusChanged, this,
            &AirPodsTrayApp::updateBatteryTooltip);
    connect(this, &AirPodsTrayApp::batteryStatusChanged, this,
            &AirPodsTrayApp::updateTrayIcon);
    connect(this, &AirPodsTrayApp::earDetectionStatusChanged, this,
            &AirPodsTrayApp::handleEarDetection);
    ...
    connect(discoveryAgent, &QBluetoothDeviceDiscoveryAgent::deviceDiscovered,
            this, &AirPodsTrayApp::onDeviceDiscovered);
    connect(discoveryAgent, &QBluetoothDeviceDiscoveryAgent::finished, this,
            &AirPodsTrayApp::onDiscoveryFinished);
    ...
    connect(&localDevice, &QBluetoothLocalDevice::deviceConnected, this,
            &AirPodsTrayApp::onDeviceConnected);
    connect(&localDevice, &QBluetoothLocalDevice::deviceDisconnected, this,
            &AirPodsTrayApp::onDeviceDisconnected);
    ...
    for (const QBluetoothAddress &address : connectedDevices) {
      ...
      if (device.serviceUuids().contains(
    ...
    connect(phoneSocket, &QBluetoothSocket::readyRead, this,
            &AirPodsTrayApp::onPhoneDataReceived);

    QDBusInterface iface("org.bluez", "/org/bluez", "org.bluez.Adapter1");
    QDBusReply<QVariant> reply =
        iface.call("GetServiceRecords",
                   QString::fromUtf8("74ec2172-0bad-4d01-8f77-997b2be0722a"));
    if (reply.isValid()) {
 ...
 private:
  ...
  void initializeDBus() {
    ...
    systemBus.connect(
        QString(), QString(), "org.freedesktop.DBus.Properties",
        "PropertiesChanged", this,
        SLOT(onDevicePropertiesChanged(QString, QVariantMap, QStringList)));

    systemBus.connect(QString(), QString(),
  ...
  void onDevicePropertiesChanged(const QString &interface,
    ...
    if (changed.contains("Connected")) {
      ...
      LOG_INFO(QString("Device %1 connected: %2")
                   .arg(devicePath, connected ? "Yes" : "No"));

      if (connected) {
        const QBluetoothAddress address =
            QBluetoothAddress(devicePath.split("/").last().replace("_", ":"));
        QBluetoothDeviceInfo device(address, "", 0);
        if (device.serviceUuids().contains(
                QBluetoothUuid("74ec2172-0bad-4d01-8f77-997b2be0722a"))) {
          connectToDevice(device);
 ...
 public slots:
  ...
  void showAvailableDevices() {
    ...
    QString selectedDevice = QInputDialog::getItem(
        nullptr, "Select Device", "Devices:", devices, 0, false, &ok);
  ...
  void setConversationalAwareness(bool enabled) {
    LOG_INFO("Setting conversational awareness to: " << (enabled ? "enabled"
                                                                 : "disabled"));
    QByteArray packet = enabled ? QByteArray::fromHex("0400040009002801000000")
                                : QByteArray::fromHex("0400040009002802000000");
    if (socket && socket->isOpen()) {
      socket->write(packet);
      LOG_DEBUG("Conversational awareness packet written: " << packet.toHex());
    } else {
      LOG_ERROR(
          "Socket is not open, cannot write conversational awareness packet");
    }
  }
  ...
  void activateA2dpProfile() {
    ...
    int result = QProcess::execute(
        "pactl", QStringList()
                     << "set-card-profile"
                     << "bluez_card." +
                            connectedDeviceMacAddress.replace(":", "_")
                     << "a2dp-sink");
    if (result != 0) {
 ...
 private slots:
  ...
  void onDeviceDiscovered(const QBluetoothDeviceInfo &device) {
    ...
    LOG_INFO("Device discovered: " << device.name() << " ("
  ...
  void onDeviceDisconnected(const QBluetoothAddress &address) {
    ...
    if (phoneSocket && phoneSocket->isOpen()) {
      ...
      LOG_DEBUG("AIRPODS_DISCONNECTED packet written: "
                << airpodsDisconnectedPacket.toHex());
    }
  }

  void connectToDevice(const QBluetoothDeviceInfo &device) {
    if (socket && socket->isOpen() &&
        socket->peerAddress() == device.address()) {
    ...
    connect(localSocket, &QBluetoothSocket::connected, this,
            [this, localSocket]() {
              LOG_INFO("Connected to device, sending initial packets");
              discoveryAgent->stop();

              QByteArray handshakePacket =
                  QByteArray::fromHex("00000400010002000000000000000000");
              QByteArray setSpecificFeaturesPacket =
                  QByteArray::fromHex("040004004d00ff00000000000000");
              QByteArray requestNotificationsPacket =
                  QByteArray::fromHex("040004000f00ffffffffff");

              qint64 bytesWritten = localSocket->write(handshakePacket);
              LOG_DEBUG("Handshake packet written: " << handshakePacket.toHex()
                                                     << ", bytes written: "
                                                     << bytesWritten);
              localSocket->write(setSpecificFeaturesPacket);
              LOG_DEBUG("Set specific features packet written: "
                        << setSpecificFeaturesPacket.toHex());
              localSocket->write(requestNotificationsPacket);
              LOG_DEBUG("Request notifications packet written: "
                        << requestNotificationsPacket.toHex());
              connect(localSocket, &QBluetoothSocket::bytesWritten, this,
                      [this, localSocket, setSpecificFeaturesPacket,
                       requestNotificationsPacket](qint64 bytes) {
              ...
              connect(localSocket, &QBluetoothSocket::readyRead, this,
                      [this, localSocket]() {
                        ...
                        QMetaObject::invokeMethod(this, "parseData",
                                                  Qt::QueuedConnection,
              ...
              QTimer::singleShot(
                  500, this,
                  [localSocket, setSpecificFeaturesPacket,
                   requestNotificationsPacket]() {
                    if (localSocket->isOpen()) {
                      localSocket->write(setSpecificFeaturesPacket);
                      LOG_DEBUG("Resent set specific features packet: "
                                << setSpecificFeaturesPacket.toHex());
                      localSocket->write(requestNotificationsPacket);
                      LOG_DEBUG("Resent request notifications packet: "
                                << requestNotificationsPacket.toHex());
                    } else {
    ...
    connect(localSocket,
            ...
            this, [this, localSocket](QBluetoothSocket::SocketError error) {
  ...
  void parseData(const QByteArray &data) {
    ...
    if (data.size() == 11 &&
        data.startsWith(QByteArray::fromHex("0400040009000D"))) {
      int mode = data[7] - 1;
      ...
      if (mode >= 0 && mode <= 3) {
        ...
        LOG_ERROR("Invalid noise control mode value received: " << mode);
               ...
               data.startsWith(QByteArray::fromHex("040004000600"))) {
      char primary = data[6];
      char secondary = data[7];
      QString earDetectionStatus =
          QString("Primary: %1, Secondary: %2")
      ...
      emit earDetectionStatusChanged(earDetectionStatus);
               ...
               data.startsWith(QByteArray::fromHex("040004000400"))) {
      ...
      emit batteryStatusChanged(batteryStatus);
               ...
               data.startsWith(QByteArray::fromHex("040004004B00020001"))) {
  ...
  void handleConversationalAwareness(const QByteArray &data) {
    ...
    LOG_INFO(
        "Conversational awareness: " << (lowered ? "enabled" : "disabled"));
  ...
  void initializeMprisInterface() {
    ...
    if (!mprisService.isEmpty()) {
      mprisInterface = new QDBusInterface(
          mprisService, "/org/mpris/MediaPlayer2",
          "org.mpris.MediaPlayer2.Player", QDBusConnection::sessionBus(), this);
      if (!mprisInterface->isValid()) {
        LOG_ERROR("Failed to initialize MPRIS interface for service: "
                  << mprisService);
      } else {
  ...
  void onInterfacesAdded(QString path, QVariantMap interfaces) {
    if (interfaces.contains("org.bluez.Device1")) {
      ...
      if (deviceProps.contains("Connected") &&
 ...
 public:
  ...
  void sendDisconnectRequestToAndroid() {
    if (phoneSocket && phoneSocket->isOpen()) {
      ...
      LOG_DEBUG(
          "Sent disconnect request to Android: " << disconnectRequest.toHex());
    } else {
  ...
  void forceL2capConnection(const QBluetoothAddress &address) {
    ...
    while (timer.elapsed() < 10000) {
      QProcess bcProcess;
      bcProcess.start("bluetoothctl", QStringList()
    ...
    LOG_ERROR("Failed to connect to device within 10 seconds: "
              << address.toString());
  }

  void initializeBluetooth() { connectToPhone(); }
...
int main(int argc, char *argv[]) {
  ...
  QObject::connect(&trayApp, &AirPodsTrayApp::noiseControlModeChanged, &engine,
                   [&engine](int mode) {
                     QObject *rootObject = engine.rootObjects().constFirst();

                     if (rootObject) {
                       QObject *noiseControlMode =
                           rootObject->findChild<QObject *>("noiseControlMode");
                       if (noiseControlMode) {
                         if (mode >= 0 && mode <= 3) {
                           QMetaObject::invokeMethod(noiseControlMode,
                                                     "setCurrentIndex",
                                                     Q_ARG(int, mode));
                         } else {
  ...
  QObject::connect(
      &trayApp, &AirPodsTrayApp::earDetectionStatusChanged,
      [&engine](const QString &status) {
        QObject *rootObject = engine.rootObjects().first();
        if (rootObject) {
          QObject *earDetectionStatus =
              rootObject->findChild<QObject *>("earDetectionStatus");
          if (earDetectionStatus) {
            earDetectionStatus->setProperty("text",
                                            "Ear Detection Status: " + status);
          }
        } else {
          LOG_ERROR("Root object not found");
        }
  ...
  QObject::connect(&trayApp, &AirPodsTrayApp::batteryStatusChanged,
                   [&engine](const QString &status) {
                     QObject *rootObject = engine.rootObjects().first();
                     if (rootObject) {
                       QObject *batteryStatus =
                           rootObject->findChild<QObject *>("batteryStatus");
                       if (batteryStatus) {
                         batteryStatus->setProperty(
